...
function makeEvent({ path, method, body = null, sessionId = null }) {
  return {
    ...
    requestContext: {
      ...
      identity: { sourceIp: '198.51.100.21' }
    }
  };
}
...
beforeEach(() => {
  ...
  sessionState = {
    sessionId: '6d6e7efb-6d0a-4d11-87e5-f98f8d4465de',
    ipHash: 'hashed-ip',
    status: 'created',
    triesLeft: 3,
    personImageUrl: null,
  ...
  mockUpdateValidation.mockImplementation(async (_sessionId, _ip, validationResult, imageUrl) => {
  ...
  mockSavePredictionId.mockImplementation(async (_sessionId, _ip, predictionId) => {
    sessionState.predictionId = predictionId;
    sessionState.updatedAt = Date.now();
    return { ...sessionState };
  });

  mockSaveResult.mockImplementation(async (_sessionId, _ip, resultUrl) => {
  ...
  mockMarkFailed.mockImplementation(async (_sessionId, _ip, message) => {
  ...
  mockSubmitTryOnRequest.mockResolvedValue({
    predictionId: 'pred_999',
    status: 'processing'
  });
  mockCheckPredictionStatus.mockResolvedValue({
    status: 'completed',
    output: ['https://wishlize-results-mumbai.s3.ap-south-1.amazonaws.com/results/tryon-2.jpg']
  });
});

describe('handler flow integration', () => {
  test('processes happy path and supports status path parameter', async () => {
    const validateEvent = makeEvent({
      path: '/validate-photo',
      method: 'POST',
      body: {
        sessionId: sessionState.sessionId,
        imageUrl: 'https://wishlize-uploads-mumbai.s3.ap-south-1.amazonaws.com/uploads/session/photo.jpg'
      }
    });
    ...
    expect(mockSavePredictionId).toHaveBeenCalledTimes(1);
  ...
  test('does not consume quota when FASHN submission fails', async () => {
    ...
    expect(mockSavePredictionId).not.toHaveBeenCalled();
